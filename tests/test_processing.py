...
@pytest.mark.parametrize("filter_list, state, expected", [
    ([], "", []),
    (test_list, "CANCELED",
    [{'id': 594226727, 'state': 'CANCELED', 'date': '2018-09-12T21:27:25.241689'},
     {'id': 615064591, 'state': 'CANCELED', 'date': '2018-10-14T08:21:33.419441'},
...
def test_filter_by_state(filter_list, state, expected):
    assert filter_by_state(filter_list, state) == expected


@pytest.mark.parametrize("sort_list, test_sort, expected", [
    (test_list, True,
...
def test_sort_by_date(sort_list, test_sort, expected):
